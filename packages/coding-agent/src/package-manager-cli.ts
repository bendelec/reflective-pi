...
function getPackageCommandUsage(command: PackageCommand): string {
	switch (command) {
		case "install":
			return `${BINARY_NAME} install <source> [-l] [--approve|--no-approve]`;
		case "remove":
			return `${BINARY_NAME} remove <source> [-l] [--approve|--no-approve]`;
		case "update":
			return `${BINARY_NAME} update [source|self|rxpi] [--self|--extensions|--models|--all] [--extension <source>] [--approve|--no-approve] [--force]`;
		case "list":
			return `${BINARY_NAME} list [--approve|--no-approve]`;
	}
}

const CONFIG_COMMAND_USAGE = `${BINARY_NAME} config [-l] [--approve|--no-approve]`;
...
Examples:
  ${BINARY_NAME} install npm:@foo/bar
  ${BINARY_NAME} install git:github.com/user/repo
  ${BINARY_NAME} install git:git@github.com:user/repo
  ${BINARY_NAME} install https://github.com/user/repo
  ${BINARY_NAME} install ssh://git@github.com/user/repo
  ${BINARY_NAME} install ./local/path
`);
			return;
...
Alias: ${BINARY_NAME} uninstall <source> [-l]
...
Examples:
  ${BINARY_NAME} remove npm:@foo/bar
  ${BINARY_NAME} uninstall npm:@foo/bar
`);
			return;
...
Update rxpi, installed packages, or model catalogs.

Options:
  --self                  Update rxpi only (default when no target is given)
  --extensions            Update installed packages only
  --models                Refresh model catalogs only
  --all                   Update rxpi and installed packages
  --extension <source>    Update one package only
  -a, --approve           Trust project-local files for this command
  -na, --no-approve       Ignore project-local files for this command
  --force                 Reinstall rxpi even if the current version is latest

Short forms:
  ${BINARY_NAME} update                Update rxpi only
  ${BINARY_NAME} update --all          Update rxpi and all extensions
  ${BINARY_NAME} update --models       Refresh model catalogs only
  ${BINARY_NAME} update <source>       Update one package
  ${BINARY_NAME} update rxpi             Update rxpi only (self works as alias to rxpi)
`);
			return;
...
function printSelfUpdateUnavailable(
	...
	console.error(`error: ${BINARY_NAME} cannot self-update this installation.`);
	console.error(getSelfUpdateUnavailableInstruction(PACKAGE_NAME, npmCommand, updatePackageTarget));

	const entrypoint = process.argv[1];
	if (entrypoint) {
		console.error("");
		console.error(`Location of ${BINARY_NAME} executable: ${entrypoint}`);
	}
}
...
function printPnpmSelfUpdateMetadataHint(): void {
	...
	console.error(chalk.yellow(`Run \`pnpm store prune\` and retry \`${BINARY_NAME} update --self\`.`));
}
...
async function getSelfUpdatePlan(force: boolean): Promise<SelfUpdatePlan> {
	...
	try {
		...
		throw new Error(`Could not determine latest ${BINARY_NAME} version: ${formatVersionCheckError(error)}`, {
			cause: error,
		});
	}
	if (!latestRelease) {
		throw new Error(`Could not determine latest ${BINARY_NAME} version.`);
	}
	...
	console.log(chalk.green(`${BINARY_NAME} is already up to date (v${VERSION})`));
	return { packageName, installSpec, version: latestRelease.version, shouldRun: false };
}

async function runSelfUpdate(command: SelfUpdateCommand): Promise<void> {
	console.log(chalk.dim(`Updating ${BINARY_NAME} with ${command.display}...`));
...
export async function handleConfigCommand(
	...
	for (const arg of rest) {
		if (arg === "-l" || arg === "--local") {
			...
			console.error(chalk.dim(`Use "${BINARY_NAME} --help" or "${CONFIG_COMMAND_USAGE}".`));
			process.exitCode = 1;
			return true;
		} else {
...
export async function handlePackageCommand(
	...
	if (options.invalidOption) {
		...
		console.error(chalk.dim(`Use "${BINARY_NAME} --help" or "${getPackageCommandUsage(options.command)}".`));
		process.exitCode = 1;
		return true;
	}
		...
		switch (options.command) {
			...
			case "update": {
				...
				if (options.showExtensionsSkippedNote) {
					console.log(
						chalk.dim(`Extensions are skipped. Run ${BINARY_NAME} update --extensions to update extensions.`),
					);
				}
				...
				if (updateTargetIncludesSelf(target)) {
					...
					if (process.platform === "win32" && installMethod !== "npm" && installMethod !== "pnpm") {
						console.error(
							chalk.red(`${BINARY_NAME} self-update on Windows is only supported for npm and pnpm installs.`),
						);
						console.error(
							chalk.dim(`Detected install method: ${installMethod}. Update ${BINARY_NAME} manually.`),
						);
						process.exitCode = 1;
						return true;
					}
					...
					console.log(chalk.green(`Updated ${BINARY_NAME} from ${VERSION} to ${selfUpdatePlan.version}`));
				}
				return true;
			}
		}
