...
describe("issue #7253: manual compaction during an active response", () => {
	...
	it("persists the aborted response before running the requested manual compaction", async () => {
		...
		const harness = await createHarness({
			// The reflective-context fork adds tool definitions and a baseline context-status
			// message to every request; a larger faux window keeps the threshold compaction
			// out of this manual-compaction regression scenario.
