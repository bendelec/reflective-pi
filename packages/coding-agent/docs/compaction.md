...
| `reserveTokens` | `16384` | Tokens to reserve for LLM response. Raising it also lowers the `[context-status]` hygiene threshold, which tracks ten points below the compaction line (clamped to [50%, 80%]) — see [reflective-context.md](reflective-context.md) |
