 ...
 *   --analyze      Spawn rxpi subagents to analyze each transcript file
...
import { BINARY_NAME } from "../packages/coding-agent/src/config.ts";
...
function runSubagent(prompt: string, cwd: string): Promise<{ success: boolean }> {
	return new Promise((resolve) => {
		const child = spawn(BINARY_NAME, ["--mode", "json", "--tools", "read,write", "-p", prompt], {
			cwd,
		...
		child.on("error", (err) => {
			console.error(chalk.red(`  Failed to spawn ${BINARY_NAME}: ${err.message}`));
...
async function main() {
	...
	if (!analyzeFlag) {
		console.log("\nRun with --analyze to spawn rxpi subagents for pattern analysis.");
		return;
	}
