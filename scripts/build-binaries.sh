#!/usr/bin/env bash
#
# Build rxpi binaries for all platforms locally.
...
#     rxpi-darwin-arm64.tar.gz
#     rxpi-darwin-x64.tar.gz
#     rxpi-linux-x64.tar.gz
#     rxpi-linux-arm64.tar.gz
#     rxpi-windows-x64.zip
#     rxpi-windows-arm64.zip
...
for platform in "${PLATFORMS[@]}"; do
    if [[ "$platform" == windows-* ]]; then
        # Windows (zip)
        echo "Creating rxpi-$platform.zip..."
        (cd "$platform" && zip -r ../rxpi-$platform.zip .)
    else
        # Unix platforms (tar.gz) - use wrapper directory for mise compatibility
        echo "Creating rxpi-$platform.tar.gz..."
        mv "$platform" rxpi && tar -czf rxpi-$platform.tar.gz rxpi && mv rxpi "$platform"
    fi
done
...
for platform in "${PLATFORMS[@]}"; do
    rm -rf "$platform"
    if [[ "$platform" == windows-* ]]; then
        mkdir -p "$platform" && (cd "$platform" && unzip -q ../rxpi-$platform.zip)
    else
        tar -xzf rxpi-$platform.tar.gz && mv rxpi "$platform"
    fi
done
...
for platform in "${PLATFORMS[@]}"; do
    if [[ "$platform" == windows-* ]]; then
        echo "  $OUTPUT_DIR/$platform/rxpi.exe"
    else
        echo "  $OUTPUT_DIR/$platform/rxpi"
    fi
done
