...
function buildBunBinaryRelease(targetDirectory, archiveDirectory) {
	...
	const archiveName = platform.startsWith("windows-") ? `rxpi-${platform}.zip` : `rxpi-${platform}.tar.gz`;
	cpSync(join(binaryBuildDirectory, archiveName), join(archiveDirectory, archiveName));
	return platform;
}

function createRxpiShim(installDirectory) {
	const binDirectory = join(installDirectory, "node_modules", ".bin");
	if (process.platform === "win32") {
		if (existsSync(join(binDirectory, "rxpi.cmd"))) {
			writeFileSync(join(installDirectory, "rxpi.cmd"), '@ECHO off\r\n"%~dp0node_modules\\.bin\\rxpi.cmd" %*\r\n');
			writeFileSync(join(installDirectory, "rxpi.ps1"), '& "$PSScriptRoot/node_modules/.bin/rxpi.ps1" @args\n');
			return;
		}
		writeFileSync(join(installDirectory, "rxpi.cmd"), '@ECHO off\r\n"%~dp0node_modules\\.bin\\rxpi.exe" %*\r\n');
		writeFileSync(join(installDirectory, "rxpi.ps1"), '& "$PSScriptRoot/node_modules/.bin/rxpi.exe" @args\n');
		return;
	}
	symlinkSync(join("node_modules", ".bin", "rxpi"), join(installDirectory, "rxpi"));
}
...
if (!options.skipInstall) {
	...
	createRxpiShim(nodeInstallDirectory);
	...
	if (!options.skipBunInstall) {
		...
		createRxpiShim(bunInstallDirectory);
	}
}
...
if (!options.skipInstall) {
	...
	console.log(`  ${join(outDir, `rxpi-${binaryPlatform}.${String(binaryPlatform).startsWith("windows-") ? "zip" : "tar.gz"}`)}`);
	console.log("\nRun the local Bun binary release from outside the repository:");
	console.log(`  ${join(binaryDirectory, String(binaryPlatform).startsWith("windows-") ? "rxpi.exe" : "rxpi")} --help`);

	console.log("\nIsolated npm install:");
	console.log(`  ${nodeInstallDirectory}`);
	console.log("\nRun the locally packed npm CLI from outside the repository:");
	console.log(`  ${join(nodeInstallDirectory, process.platform === "win32" ? "rxpi.cmd" : "rxpi")} --help`);
	...
	if (!options.skipBunInstall) {
		...
		console.log(`  ${join(bunInstallDirectory, process.platform === "win32" ? "rxpi.cmd" : "rxpi")} --help`);
	}
}
